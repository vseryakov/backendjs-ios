...
{
    [BKjs invoke:self name:self.type arg:transitionContext];
}

- (void)viewTransition:(id<UIViewControllerContextTransitioning>)transitionContext
{
...
- (void)explode:(id<UIViewControllerContextTransitioning>)transitionContext
{
    ...
    [UIView animateWithDuration:duration animations:^{
        for (UIView *view in snapshots) {
            CGFloat xOffset = [BKjs rand:-100.0 to:100.0];
            CGFloat yOffset = [BKjs rand:-100.0 to:100.0];
            view.frame = CGRectOffset(view.frame, xOffset, yOffset);
            view.alpha = 0.0;
            view.transform = CGAffineTransformScale(CGAffineTransformMakeRotation([BKjs rand:-10.0 to:10.0]), 0.0, 0.0);
        }
    } completion:^(BOOL finished) {
        for (UIView *view in snapshots) {
...
- (NSArray*) valueArrayForStartValue:(CGFloat)startValue endValue:(CGFloat)endValue
{
	...
	if ([self.stiffness isEqual:@"Heavy"]) {
		stiffnessCoefficient = 0.001f;
	} else
    if ([self.stiffness isEqual:@"Light"]) {
...
@implementation BKGlowAnimation
